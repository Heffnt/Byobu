...
- **Named frequencies** — 17 richer tones, each with an emblem (flame, sparkle,
  ...
  **decomposes** into a multiset of simpler frequencies (a directed graph). The
  four highest — Laternical, Malvesian, Thurmistic, Saspacian — are **legendary**:
...
- **Plus (⊕)** — lets the brewer *add* one frequency of their choice (only
  Southollow Royal Tulip, ×1). This is the **only** way to manifest a legendary
  frequency.
...
- `recipes[]`: `{id, name, school, desc, req[], example[], trim, wildAdd, tier}`
  — `req` is the required frequency multiset; `example` is one valid ingredient
  set; `tier` ∈ {simple, advanced, legendary}.
...
- **Self-verify before finishing**: Chromium is available. Drive the page with
  ...
  - A legendary recipe (e.g. **Cosmic Saspacian No. 5**) becomes CRAFTABLE only
    when **Southollow Royal Tulip** (⊕) is in the brew.
